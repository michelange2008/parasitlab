...
$("#troupeau_animal_create").attr('disabled', true);
...
$("#eleveur").on('change', function() {
  ...
  .done(function(datas) {
    ...
    if (nb_troupeau == 0) {
      $("#troupeau_animal_create").empty().append('<option>Aucun troupeau: il faut en ajouter un --></option>').attr('disabled',true);
    }
    else {
      var options = '';
      ...
      $("#troupeau_animal_create").empty().append(options).attr('disabled', false);
